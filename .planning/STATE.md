...
- **✅ ACCEPTED by WordPress.org 2026-06-16.** Slug **confirmed `maestro-menu-editor`** (matches text domain — no code change). SVN URL `https://plugins.svn.wordpress.org/maestro-menu-editor`; commit access activates within ~1h of approval. **`bin/deploy-svn.sh` is written and syntax-checked** — it builds the runtime tree, checks out the repo to `build/svn`, syncs `trunk/` + directory `assets/`, copies the `tags/1.0.0` tag, then **stops before `svn ci`** (user runs the credentialed commit: SVN username = WP.org username `dpknauss`, separate SVN password from the profile page). Off-sandbox, so it runs on the user's machine once access is live.
