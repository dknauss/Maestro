...
- [ ] V2-17 — single-site privileged editor tier (edges toward the Out-of-Scope “page locking” line; an enforced tier is out of scope for Maestro — Maestro never enforces and assumes no dependency on any other plugin, so any enforced tier would be separate work in a separate project, not Maestro core)
